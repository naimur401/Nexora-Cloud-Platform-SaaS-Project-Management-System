...
router.get('/companies', isSuperAdmin, async (req, res) => {
  try {
    const result = await pool.query(
      'SELECT c.*, COUNT(DISTINCT u.id) as user_count, COUNT(DISTINCT p.id) as project_count FROM companies c LEFT JOIN users u ON u.company_id = c.id LEFT JOIN projects p ON p.company_id = c.id GROUP BY c.id ORDER BY c.created_at DESC'
    );
...
router.post('/companies', isSuperAdmin, async (req, res) => {
  try {
    ...
    const result = await pool.query('INSERT INTO companies (name) VALUES () RETURNING *', [name]);
...
router.get('/users', isSuperAdmin, async (req, res) => {
  try {
    ...
    let query = 'SELECT u.id, u.name, u.email, u.role, u.company_id, c.name as company_name, u.created_at FROM users u LEFT JOIN companies c ON c.id = u.company_id WHERE u.role != ';
    const params: any[] = ['SUPER_ADMIN'];
    let paramCount = 2;
    if (companyId) {
...
router.put('/users/:id/role', isSuperAdmin, async (req, res) => {
  try {
    ...
    const result = await pool.query('UPDATE users SET role =  WHERE id =  RETURNING id, name, email, role', [role, id]);
...
router.get('/logs', isSuperAdmin, async (req, res) => {
  try {
    await pool.query(
      'CREATE TABLE IF NOT EXISTS audit_logs (id SERIAL PRIMARY KEY, user_id INTEGER, action VARCHAR(255), details JSONB, created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP)'
    );
    const result = await pool.query(
      'SELECT al.*, u.name as user_name, u.email as user_email FROM audit_logs al LEFT JOIN users u ON u.id = al.user_id ORDER BY al.created_at DESC LIMIT 50'
    );
    res.json({ success: true, data: result.rows });
  } catch (error) {
